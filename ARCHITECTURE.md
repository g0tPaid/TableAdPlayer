...
5. **Heartbeats and events are queued.** Heartbeats persist in Room `playback_events` (`type=heartbeat`) and drain from `ReportingRepository`. Playback-event drain is Phase 7. Reporting **never** blocks `PlaylistEngine`.
...
com.tableadplayer.app
  ...
  reporting/     In-memory queue leftover; heartbeats use Room outbox
  kiosk/         Boot receiver (also enqueues sync)
```
